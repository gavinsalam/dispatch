! io_utils.f90
!
! $Id: io_utils.f90,v 1.1 2001/10/19 18:00:27 gsalam Exp $
!
!======================================================================
...
module sub_defs_io
  ...
  interface
     function command_line()
       implicit none
       character(len=600) ::  command_line
  ...
  interface
     function string_val_opt(opt,default_val)
       implicit none
       character(len=180)                     :: string_val_opt
       character(len=*), intent(in)           :: opt
...
!----------------------------------------------------------------------
subroutine open_arg(iarg, idev, ext, status, form, default_val)
  use sub_defs_io, only : lcl_getarg, lcl_iargc
  use track_args
  implicit none
  integer, intent(in) :: iarg, idev
  character(*), optional, intent(in) :: ext, status, form, default_val
